...
//go:generate mockgen -destination=./mock_subject.go -source=subject.go SubjectStore
...
type SubjectManager struct {
	list    SubjectList    // actual storage
...
func (sm *SubjectManager) save(info SubjectInfo) {
...
func (sm *SubjectManager) update(info SubjectInfo) {
	k := sm.idMap[info.ID]
	p := &sm.list[k]
	if p.Key != info.Key {
		delete(sm.keyMap, p.Key)
...
func (sm *SubjectManager) get(id int) (SubjectInfo, error) {
	if val, ok := sm.idMap[id]; ok {
		return sm.list[val], nil
	} else {
		return SubjectInfo{}, errNotExist
	}
}

func (sm *SubjectManager) Init(list SubjectList) {
